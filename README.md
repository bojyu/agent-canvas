...
选中至少 2 个未编组节点后按 `Ctrl/Cmd + G` 可创建群组。浅色半透明外框会包围组内节点，拖动外框可整体移动，组内节点仍能单独编辑；群组被选中时会显示低对比度的边缘缩放点，可手动调整外框大小，将成员拖向边界时外框也会自动扩展。选中群组后点击标题栏的小铅笔，或直接双击群组名称即可重命名，回车或失焦保存、`Esc` 取消。选中外框或任一组内节点后按 `Ctrl/Cmd + Backspace` 会打散整个群组并保留节点与连线。群组不支持嵌套；复制群组会保留成员关系但不复制连线，删除群组会同时删除其成员，并可通过 `Ctrl/Cmd + Z` 撤销。保存、另存为和分享文件会保留群组名称与编组关系，“整理”只排列未编组节点，不改变群组内部布局。
