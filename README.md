...
顶部“主题”可以切换“跟随系统”“浅色”或“深色”。主题偏好保存在当前浏览器本机并作用于整个 Agent Canvas，包括画布、节点、群组、节点库、任务中心和设置弹窗；它不会写进画布项目或分享文件。深色模式只改变界面颜色，不会对图片与视频预览套用反色滤镜。
...
使用 `Ctrl/Cmd + C`、`Ctrl/Cmd + V` 或按住 `Alt` 拖动复制节点时，只复制节点自身内容和配置，不复制任何连线，也不会继承原节点的 Agent 会话或运行时端口状态。`Alt` 拖动时鼠标带走的是新副本，原节点及其连线在松开后保持原位；多选节点遵循相同规则。画布“另存为”和导出分享仍会完整保留节点间连线。

选中至少 2 个未编组节点后按 `Ctrl/Cmd + G` 可创建群组。把未编组节点拖到群组内并与外框重叠 40% 以上时，外框会高亮，松开即可加入并按需要自动扩展；也可以同时选中一个群组和若干未编组节点，再按 `Ctrl/Cmd + G` 批量加入。选中组内成员后按 `Ctrl/Cmd + Shift + G` 只将这些成员移出并保持绝对位置和连线，选中外框或任一成员后按 `Ctrl/Cmd + Backspace` 则打散整个群组。浅色半透明外框可整体移动和低对比度缩放，标题栏的小铅笔或双击名称可重命名。群组不支持嵌套；复制完整群组会保留成员关系和内部连线，删除群组会同时删除其成员，所有成员调整均可通过 `Ctrl/Cmd + Z` 撤销。保存、另存为和分享文件会保留群组名称与编组关系，“整理”只排列未编组节点，不改变群组内部布局。
...
参考图和图片输出统一为“图片”节点，参考视频和视频输出统一为“视频”节点。两种媒体节点都在左右提供输入、输出连接点：既能作为生成节点的参考素材，也能接收、预览、下载生成结果，并继续传给下游节点。双击图片节点的非按钮区域会打开系统图片选择器；图片节点也支持点击“粘贴剪贴板图片”，或在选中单个图片节点后直接按 `Ctrl/Cmd + V` 粘贴截图和从其他应用复制的图片。点击预览图本身不会下载；鼠标悬停预览图后，只有点击浮动下载按钮才会保存图片。旧画布里的图片输出和视频输出节点会自动迁移为对应的统一媒体节点。
...
页面统一使用本地 Geist 字体、浅色工具栏、固定左侧节点栏和暖灰无限画布。画布没有平移边界，最小可缩放至 6%，方便在大型工作流中获得接近无限画布的全局俯视效果。节点栏会在展开时为画布让出空间，收起后只保留窄控制条；开合与节点加入反馈由 GSAP 驱动，并兼容“减少动态效果”系统设置。
